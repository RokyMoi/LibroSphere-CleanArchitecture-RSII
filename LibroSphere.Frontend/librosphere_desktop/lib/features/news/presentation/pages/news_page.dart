...
class NewsPage extends StatefulWidget {
  const NewsPage({super.key, required this.viewModel});
...
class _NewsPageState extends State<NewsPage> {
  ...
  Future<void> _openCreateDialog() async {
    final result = await showDialog<bool>(
      ...
      builder: (_) => _AddNewsDialog(viewModel: widget.viewModel),
    );
    if (!mounted || result != true) return;
    _showSnackBar('News article created successfully.', isError: false);
  }

  Future<void> _deleteNews(AdminNewsModel news) async {
    ...
    final result = await widget.viewModel.deleteNews(news);
    if (!mounted) return;
    switch (result) {
      case Success<void>():
        _showSnackBar('News deleted.', isError: false);
  ...
  void _showSnackBar(String msg, {required bool isError}) {
    ...
    messenger
      ...
      ..showSnackBar(SnackBar(
        content: Text(msg),
        backgroundColor:
            isError ? const Color(0xFFB42318) : const Color(0xFF1F8B4C),
      ));
  }
              ...
              Expanded(
                child: AdminPanel(
                  child: vm.newsList.isEmpty
                      ? const AdminEmptyState('No news articles found.')
                      : ListView.separated(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return _NewsRow(
                              news: news,
                              isDeleting: vm.deletingNewsId == news.id,
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                if (news.createdOnUtc != null)
                  Padding(
                    padding: const EdgeInsets.only(top: 6),
                    child: Text(
                      formatAdminDateTime(news.createdOnUtc),
                      style: const TextStyle(
                        color: desktopMutedForeground,
                        fontSize: 11,
                      ),
...
class _AddNewsDialog extends StatefulWidget {
  const _AddNewsDialog({required this.viewModel});
...
class _AddNewsDialogState extends State<_AddNewsDialog> {
  ...
  Future<void> _save() async {
    ...
    if (_selectedImagePath != null) {
      ...
      final uploadResult = await widget.viewModel.uploadNewsImage(
        imageBytes: bytes,
    ...
    final result = await widget.viewModel.createNews(
