      ...
      builder: (context, snapshot) {
        ...
        return ListView(
          ...
          children: [
            ...
            Row(
              children: [
                const Text('Total Price:', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w700)),
                const Spacer(),
                Text('\$${total.toStringAsFixed(2)}', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w800)),
              ],
            ),
...
class _CheckoutScreenState extends State<CheckoutScreen> {
  ...
  void initState() {
    super.initState();
    // Odgađamo čitanje SessionScope do prvog frame-a jer initState
    // se poziva prije nego što je context potpuno montiran u stablu.
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted) return;
      final session = SessionScope.read(context);
      _nameController.text = session.currentUser?.fullName ?? '';
      _emailController.text = session.currentUser?.email ?? '';
    });
  }
  ...
  Future<void> _pay() async {
    ...
    try {
      final order = await SessionScope.read(context).checkout(
        BillingDetails(
  ...
  bool _validate() {
    ...
    final cardComplete = _cardDetails?.complete ?? false;
    ...
    setState(() {
      ...
      _cardError = cardComplete ? null : 'Please complete your card details.';
      _formError = null;
    });
  ...
  @override
  Widget build(BuildContext context) {
    final total = context.session.cart?.total ?? 0;
    return Scaffold(
      body: SafeArea(
        child: ListView(
          padding: const EdgeInsets.fromLTRB(18, 18, 18, 24),
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      const Text('Total:', style: TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.w700)),
                      const Spacer(),
                      Text('\$${total.toStringAsFixed(2)}', style: const TextStyle(color: Colors.white, fontSize: 24, fontWeight: FontWeight.w800)),
                    ],
                  ),
                  const SizedBox(height: 18),
                  PrimaryPillButton(
