...
class ApiClient {
  ...
  Future<String?> getStripePublishableKey() async {
    ...
    final value = readString(_decodeMap(response), ['publishableKey', 'PublishableKey']);
  ...
  Future<PagedResult<BookModel>> getBooks({String? searchTerm, String? accessToken}) async {
  ...
  Future<List<BookModel>> getRecommendations(String accessToken) async {
    final response = await _get('/api/recommendations?take=5', token: accessToken);
    final decoded = jsonDecode(response.body);
    if (decoded is! List) {
      return <BookModel>[];
    }

    return decoded.whereType<Map<String, dynamic>>().map(BookModel.fromJson).toList();
  }

  Future<PagedResult<LibraryEntry>> getLibrary(String accessToken) async {
    final response = await _get('/api/library?page=1&pageSize=20', token: accessToken);
  ...
  Future<String> getReadUrl(String accessToken, String bookId) async {
    final response = await _get('/api/library/$bookId/read', token: accessToken);
  ...
  Future<void> createReview(String accessToken, String bookId, int rating, String comment) async {
    await _post('/api/reviews', token: accessToken, body: {
      'bookId': bookId,
      'rating': rating,
      'comment': comment,
    });
  ...
  Future<ShoppingCartModel> upsertCart({
    ...
    final response = await _post('/api/cart', token: accessToken, body: {
      'id': cartId,
      'userId': userId,
  ...
  Future<ShoppingCartModel> createPaymentIntent(String accessToken, String cartId) async {
  ...
  Future<OrderModel> createOrder(String accessToken, String cartId) async {
    final response = await _post('/api/orders', token: accessToken, body: {'cartId': cartId});
  ...
  Future<OrderModel> waitForPaidOrder(String accessToken, String orderId) async {
  ...
  Future<http.Response> _post(String path, {String? token, Object? body}) => _send(
        () => http.post(
          _uri(path),
  ...
  Future<http.Response> _send(
    ...
    try {
      ...
      throw AppException(
        message: 'Unable to reach LibroSphere API at $_baseUrl. Start Docker compose and expose the API on port 8080.',
      );
    } on TimeoutException {
      throw AppException(
        message: 'LibroSphere API at $_baseUrl did not respond in time.',
      );
    }
  }
    ...
    if (response.body.isNotEmpty) {
      try {
        ...
        if (decoded is Map<String, dynamic>) {
          ...
          message = readString(
            decoded,
            ['error', 'Error', 'message', 'Message', 'title', 'Title'],
            fallback: message,
          );
